...
function f(s, p)
  ...
  if i then return strung.sub(s, i, e) end
end
...
-- function strset (p)
--   local res = {s=''}
--   strung.gsub(abc, p, function (c) res.s = res.s .. c end)
--   return res.s
-- end;

-- assert(strung.len(strset('[\200-\210]')) == 11)
...
-- assert(strung.gsub("um (dois) tres (quatro)", "(%(%w+%))", strung.upper) ==
--             "um (DOIS) tres (QUATRO)")
...
local function dostrung (s) return loadstrung(s)() or "" end
-- assert(strung.gsub("alo $a=1$ novamente $return a$", "$([^$]*)%$", dostrung) ==
--             "alo  novamente 1")

-- x = strung.gsub("$x=strung.gsub('alo', '.', strung.upper)$ assim vai para $return x$",
--          "$([^$]*)%$", dostrung)
...
-- t = {}; setmetatable(t, {__index = function (t,s) return strung.upper(s) end})
